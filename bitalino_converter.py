...
def emgconverter(rawEMGSignal,VCC=3.3,nBits=10):
    ...
    rawEMGSignal = [((((ADC / 2**nBits) - (1 / 2)) * VCC ) / gain) for ADC in rawEMGSignal] #Apply conversio
...
#        pickle.dump(rawEMGSignal, f)
    ...
    dataset = []
    with open(basepath + "/ecg.txt","r") as f:
        dataset = [x for x in csv.reader(f)]
    dataset = np.array(dataset[1:])
    rawECGSignal = [float(x) for x in dataset[:,-5]]
    rawECGSignal = ecgconverter(rawECGSignal)
    plt.plot(rawECGSignal,color='green')
    plt.axhline(0)
    with open(basepath + '/convertedECG.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump(rawECGSignal, f)
